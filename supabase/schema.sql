...
CREATE TABLE public.quiz_questions (
    ...
    options JSONB,
    -- for multiple choice: ["option1", "option2", ...]
...
-- Courses policies
CREATE POLICY "Anyone can view published courses" ON public.courses FOR
SELECT USING (
        is_published = true
        OR created_by = auth.uid()
    );
CREATE POLICY "Admins can create courses" ON public.courses FOR
INSERT WITH CHECK (
        EXISTS (
            SELECT 1
...
CREATE POLICY "Admins can update own courses" ON public.courses FOR
UPDATE USING (
        created_by = auth.uid()
        AND EXISTS (
            SELECT 1
...
CREATE POLICY "Admins can delete own courses" ON public.courses FOR DELETE USING (
    created_by = auth.uid()
    AND EXISTS (
        SELECT 1
        FROM public.profiles
...
CREATE POLICY "Users can enroll in courses" ON public.enrollments FOR
INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update own enrollment progress" ON public.enrollments FOR
UPDATE USING (user_id = auth.uid());
-- Quiz attempts policies
CREATE POLICY "Users can view own quiz attempts" ON public.quiz_attempts FOR
SELECT USING (user_id = auth.uid());
CREATE POLICY "Users can create quiz attempts" ON public.quiz_attempts FOR
INSERT WITH CHECK (user_id = auth.uid());
-- Certificates policies
CREATE POLICY "Users can view own certificates" ON public.certificates FOR
SELECT USING (user_id = auth.uid());
-- Events policies
CREATE POLICY "Enrolled users can view course events" ON public.events FOR
SELECT USING (
        EXISTS (
            SELECT 1
            FROM public.enrollments
